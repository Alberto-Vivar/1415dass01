	...
	public boolean requestBroadcast(Writer report) {
		...
		do {
			try {
				...
				loggingCode(report, currentNode);
			} catch (IOException exc) {
				// just ignore
			};
			currentNode = currentNode.nextNode_;
...
@return Anwer #true when the print operation was succesful and #false otherwise
	...
	private void loggingCode(Writer report, Node currentNode)
			throws IOException {
		report.write("\tNode '");
		report.write(currentNode.name_);
				...
				if (document.message_.startsWith("!PS")) {
					...
					if (startPos >= 0) {
						...
						if (startPos >= 0) {
							...
							writeAuthorAndTitle(report, author, title);
	...
	private void writeAuthorAndTitle(Writer report, String author, String title)
			throws IOException {
...
<p><strong>Precondition:</strong> isInitialized();</p>
	...
	public void printOn (StringBuffer buf) {
		...
		Node currentNode = firstNode_;
		do {
			switchStatement(buf, currentNode);
			buf.append(" -> ");
			currentNode = currentNode.nextNode_;
	...
	private void switchStatement(StringBuffer buf, Node currentNode) {
